...
```
Step 1     任务对齐卡     主模型输出目标/约束/成功标准/视角，非阻塞    （驱动全流程）
Round 1    独立工作       每个模型从分配的视角独立分析（带成功标准）   （并行）
Round 2    交叉验证       模型互相审查发现、标记分歧                   （并行）
Round 2.5  30 秒对抗      （快速模式）一次攻击防共识偏见               （并行）
           焦点反驳       （完备模式）解决关键冲突                     （并行）
Round 3    候选综合       （完备模式）主模型提出候选方案               （主模型主导）
Round 4    对抗性挑战     （完备模式）全量检查 + 侧重分配               （并行）
Round 5    final.md + 独立复核  主模型编译 final.md，外部模型复核      （编译 + 复核）
```

**两种模式：**
- **快速模式**（2 轮 + 30 秒对抗 + 独立复核）—— 多视角 + 轻量对抗
- **完备模式**（4 轮 + R3 综合 + 独立复核）—— 完整交叉验证 + 对抗性压力测试

### 交付契约：`final.md` 是**编译产物**，不是自由作文

- 硬模板：8 字段 frontmatter + 8 节正文 + 8 项自检规则
- 交付前必须通过 `scripts/cross_review_runtime.py check-final --file final.md`
- 外部模型做独立"是否自包含"复核，不通过必须返工
- 用户读完 `final.md` 无需追问即可获得三样东西：**决策、行动、溯源**
