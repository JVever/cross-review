...
Round 2: 交叉验证       模型互相审查清洗后的发现（并行）
Round 2.5: 焦点反驳     解决关键冲突后再综合（可选）
Round 3: 候选综合       主模型提出候选方案 + 假设账本
Round 4: 对抗性挑战     按检查项分工攻破方案（并行）
```

两种模式：
- **快速模式**（2 轮）—— 相对明确的任务，需要多视角但不需要深度对抗
- **完备模式**（4 轮，含可选 2.5 轮）—— 复杂/高风险任务，需要充分的交叉验证和对抗性检验
...
Skill 会自动：
1. 让你描述任务目标和材料
2. 加载已保存的 CLI 配置，或检测可用工具（首次使用后自动记住）
3. 为每个模型分配差异化视角
4. 执行评审轮次（含输出验证），输出最终综合方案

## 核心特性

- **模型无关**：适配任意 AI CLI 工具组合
- **CLI 配置持久化**：记住你的工具和调用方式，跨会话复用
- **输出质量控制**：固定输出骨架 + 两层验证，防止输出膨胀和噪声
- **Clean 输出机制**：`--output-last-message` 去除元数据噪声；超长输出(>5000 字)可选 digest
